...
RSpec.describe 'Contacts API', type: :request do
  ...
  describe 'POST /api/v1/accounts/{account.id}/contacts' do
    ...
    let(:user_info_params) { {contact: {first_name: "User first_nm", middle_name: "Middle nm", last_name: "User last_nm", gender: "male", birth_date: "04 February 1999", city: "Los Angeles", address: "Test address", country: "US" }} }
    ...
    context 'when it is an authenticated user' do
      ...
      it 'accepts first_name, last_name, city, gender, country, address as payload and save it to contact' do
        expect do
          post "/api/v1/accounts/#{account.id}/contacts", headers: admin.create_new_auth_token,
        ...
        # custom attributes are updated
        json_response = JSON.parse(response.body)
        expect(json_response['payload']['contact'].has_key?('first_name')).to be_truthy
        expect(json_response['payload']['contact'].has_key?('last_name')).to be_truthy
        expect(json_response['payload']['contact'].has_key?('city')).to be_truthy
        expect(json_response['payload']['contact'].has_key?('address')).to be_truthy
        expect(json_response['payload']['contact'].has_key?('gender')).to be_truthy
        expect(json_response['payload']['contact'].has_key?('country')).to be_truthy
      end
