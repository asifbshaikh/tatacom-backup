...
RSpec.describe 'Twitter::CallbacksController', type: :request do
  ...
  let(:raw_response) do
    object_double('raw_response', body: 'oauth_token=1&oauth_token_secret=1&user_id=100&screen_name=tring')
  end
  ...
  describe 'GET /twitter/callback' do
    it 'creates inboxes if subscription is successful' do
      ...
      expect(account.twitter_profiles.last.inbox.name).to eq 'tring'
