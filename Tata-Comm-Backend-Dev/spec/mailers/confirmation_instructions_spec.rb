...
RSpec.describe 'Confirmation Instructions', type: :mailer do
  describe :notify do
    ...
    it 'has the correct header data' do
      expect(mail.reply_to).to contain_exactly('accounts@tring.com')
    ...
    it 'does not refer to the inviter and their account' do
      expect(mail.body).to_not match('has invited you to try out Tring!')
    end
    ...
    context 'when there is an inviter' do
      ...
      it 'refers to the inviter and their account' do
        expect(mail.body).to match(
          "#{CGI.escapeHTML(inviter_val.name)}, with #{CGI.escapeHTML(account.name)}, has invited you to try out Tring!"
        )
      end
