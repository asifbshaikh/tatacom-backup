class Conversations::FilterService < FilterService
  ...
  def conversation_query_string(current_filter, query_hash, current_index)
    attribute_key = cast_integer_to_text(query_hash)
  ...
  def custom_attribute_query(query_hash, current_index)
    ...
    if custom_attribute(attribute_key)
      " (conversations.custom_attributes ->> '#{attribute_key}')::#{attribute_data_type} #{filter_operator_value} #{query_operator} "
    else
      ' '
    end
  end

  def cast_integer_to_text(data)
    ['contains', 'does_not_contain'].include?(data[:filter_operator]) ? "#{data[:attribute_key]}::text" : data[:attribute_key]
  end
end
