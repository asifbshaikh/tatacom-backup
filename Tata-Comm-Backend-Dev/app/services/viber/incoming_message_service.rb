# ref : https://developers.viber.biz/en/docs/messaging-api/receiving-messages/#webhook-event-types
# https://developers.viber.biz/en/reference/messaging-api/#message-event
...
class Viber::IncomingMessageService
  ...
  def perform
    ...
    viber_contact_info
    return if viber_contact_info['userId'].blank?
  ...
  def message_type_non_text?(type)
    [Viber::Bot::Event::MessageType::Video, Viber::Bot::Event::MessageType::Audio, Viber::Bot::Event::MessageType::Image].include?(type)
  end
  ...
  def viber_contact_info
    @viber_contact_info ||= JSON.parse(inbox.channel.client.get_profile(params[:events].first['source']['userId']).body)
  end

  def set_contact
    contact_inbox = ::ContactBuilder.new(
      source_id: viber_contact_info['userId'],
      inbox: inbox,
  ...
  def contact_attributes
    {
      name: viber_contact_info['displayName'],
      avatar_url: viber_contact_info['pictureUrl']
    }
  end
