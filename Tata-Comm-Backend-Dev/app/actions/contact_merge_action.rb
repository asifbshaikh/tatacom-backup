class ContactMergeAction
  ...
  def validate_contacts
    return unless belongs_to_account?(@base_contact) && belongs_to_account?(@mergee_contact)
  ...
  def merge_and_remove_mergee_contact
    mergable_attribute_keys = %w[identifier name first_name middle_name last_name birth_date gender email phone_number city address country additional_attributes custom_attributes]
    base_contact_attributes = base_contact.attributes.slice(*mergable_attribute_keys).deep_compact
    mergee_contact_attributes = mergee_contact.attributes.slice(*mergable_attribute_keys).deep_compact
