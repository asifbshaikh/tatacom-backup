...
class Segmentation::AddUsersWorker < SidekiqBase
  queue_as :high

  Benchmark.bmbm do |obj|
    obj.report('AddUsersWorker runtime') do
      def perform(import_file_segment_id, account_id)
        ...
        csv.each_slice(BATCH_SIZE) do |batch|
          batch.each do |row|
            ...
            if contact.present?
              if contact.source_id.include?(segment.id)
                invalid_records << row.to_h.merge(error_message: 'Contact already present in this segment')
              else
        ...
        @import_file_segment.update(total_users: csv.size, added_users: @added_users, invalid_users: invalid_records.size, status: :completed)
