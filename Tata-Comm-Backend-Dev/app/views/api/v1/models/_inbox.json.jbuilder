...
if resource.email?
  ...
  json.smtp_auth_enable resource.channel.try(:smtp_auth_enable)
  json.maximum_send_rate resource.channel.try(:maximum_send_rate)
  json.unsubscribe_setting resource.channel.try(:unsubscribe_setting)
  json.bounces_and_complaint_tracking resource.channel.try(:bounces_and_complaint_tracking)
  json.smtp_protocol resource.channel.try(:smtp_protocol)
  json.email_api_url resource.channel.try(:email_api_url)
  json.email_api_key resource.channel.try(:email_api_key)
end
...
if resource.tata_sms?
  json.auth_token resource.channel&.decrypt_auth_token
  json.medium resource.channel.try(:medium)
  json.sender_id resource.channel.try(:sender_id)
  json.sender_type resource.channel.try(:sender_type)
  json.callback_url resource.channel.try(:callback_url)
end

if resource.tata_whatsapp?
