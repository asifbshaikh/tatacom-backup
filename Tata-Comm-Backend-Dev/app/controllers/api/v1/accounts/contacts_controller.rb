class Api::V1::Accounts::ContactsController < Api::V1::Accounts::BaseController
  ...
  def import
    ...
    ActiveRecord::Base.transaction do
      if(params[:label_name])
        # create the label first and pass label_name
        Current.account.labels.create!(title:params[:label_name],description:"auto-generated", show_on_sidebar:false)
        import = Current.account.data_imports.create!(data_type: 'contacts_with_label', label:params[:label_name])
      else
  ...
  def create
    ActiveRecord::Base.transaction do
      ...
      @contact.birth_date = Date.parse(params["birth_date"], "%d/%m/%Y") if params["birth_date"].present?
      @contact.last_activity_at = Time.current
      @contact.save!
  ...
  def contact_params
    params.require(:contact).permit(:name, :identifier, :email, :phone_number, :first_name, :middle_name, :last_name, :gender, :city, :address, :country, :birth_date, :last_activity_at, additional_attributes: {}, custom_attributes: {})
  end
