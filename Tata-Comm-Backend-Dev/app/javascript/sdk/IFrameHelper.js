...
import { TRING_ERROR, TRING_READY } from '../widget/constants/sdkEvents';
...
export const IFrameHelper = {
  ...
  createFrame: ({ baseUrl, websiteToken }) => {
    ...
    iframe.id = 'tring_live_chat_widget';
    iframe.style.visibility = 'hidden';

    let holderClassName = `woot-widget-holder woot--hide woot-elements--${window.$tring.position}`;
    if (window.$tring.hideMessageBubble) {
      holderClassName += ` woot-widget--without-bubble`;
    }
    if (isFlatWidgetStyle(window.$tring.widgetStyle)) {
  ...
  getAppFrame: () => document.getElementById('tring_live_chat_widget'),
  getBubbleHolder: () => document.getElementsByClassName('woot--bubble-holder'),
  sendMessage: (key, value) => {
    const element = IFrameHelper.getAppFrame();
    element.contentWindow.postMessage(
      `tring-widget:${JSON.stringify({ event: key, ...value })}`,
      '*'
    );
  },
  initPostMessageCommunication: () => {
    window.onmessage = e => {
      if (typeof e.data !== 'string' || e.data.indexOf('tring-widget:') !== 0) {
        return;
      }
      const message = JSON.parse(e.data.replace('tring-widget:', ''));
  ...
  events: {
    loaded: message => {
      ...
      window.$tring.hasLoaded = true;
      IFrameHelper.sendMessage('config-set', {
        locale: window.$tring.locale,
        position: window.$tring.position,
        hideMessageBubble: window.$tring.hideMessageBubble,
        showPopoutButton: window.$tring.showPopoutButton,
        widgetStyle: window.$tring.widgetStyle,
      });
      IFrameHelper.onLoad({
      ...
      if (window.$tring.user) {
        IFrameHelper.sendMessage('set-user', window.$tring.user);
      }

      dispatchWindowEvent({ eventName: TRING_READY });
      ...
      if (!window.$tring.resetTriggered) {
        dispatchWindowEvent({ eventName: TRING_READY });
      }
    },
    error: ({ errorType, data }) => {
      dispatchWindowEvent({ eventName: TRING_ERROR, data: data });
    ...
    setBubbleLabel(message) {
      ...
      setBubbleText(window.$tring.launcherTitle || message.label);
    },
  ...
  onLoad: ({ widgetColor }) => {
    ...
    iframe.setAttribute('id', `tring_live_chat_widget`);
    ...
    if (!window.$tring.hideMessageBubble) {
      ...
      let closeBtnClassName = `woot-elements--${window.$tring.position} woot-widget-bubble woot--close woot--hide`;
