...
import TataChannel from '../../api/channel/tataChannel';
import TataCommunicationsWhatsappChannel from '../../api/channel/tataCommunicationsWhatsappChannel';
...
export const getters = {
  ...
  //   );*/
  // },
  getSMSInboxes: $state => filter => {
    if(!filter) {
      return $state.records;
    }
    return $state.records.filter(
      item => item.channel_type === filter
    );
  },
...
export const actions = {
  ...
  createTataCommunicationsWhatsappChannel: async ({ commit }, params) => {
    try {
      commit(types.default.SET_INBOXES_UI_FLAG, { isCreating: true });
      const response = await TataCommunicationsWhatsappChannel.create(params);
      commit(types.default.ADD_INBOXES, response.data);
      commit(types.default.SET_INBOXES_UI_FLAG, { isCreating: false });
      return response.data;
    } catch (error) {
  ...
  createTataChannel: async ({ commit }, params) => {
    try {
      commit(types.default.SET_INBOXES_UI_FLAG, { isCreating: true });
      const response = await TataChannel.create(params);
      commit(types.default.ADD_INBOXES, response.data);
      commit(types.default.SET_INBOXES_UI_FLAG, { isCreating: false });
      return response.data;
    } catch (error) {
