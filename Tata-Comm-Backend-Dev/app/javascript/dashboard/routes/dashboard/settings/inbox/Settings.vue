<template>
  <div class="settings columns container">
    ...
    <div v-if="selectedTabKey === 'details'" class="settings--content">

      <label for="toggle-business-hours" class="toggle-input-wrap">
        WhatsApp Display Name
        <input v-model="selectedInboxNameTest" type="text" name="toggle-business-hours" />
      </label>
      <!-- <label for="toggle-business-hours" class="toggle-input-wrap">
        Messaging Limit
        <input v-model="messageLimit" type="text" name="toggle-business-hours" />
      </label>
      <label for="toggle-business-hours" class="toggle-input-wrap">
        Quality Rating
        <input v-model="qualityRating" type="text" name="toggle-business-hours" />
      </label> -->
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Default language for templates
          <input v-model="defultLang" type="text" name="toggle-business-hours" />
        </label>        
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Message on Behalf Name
          <input v-model="messageOnBehalf" type="text" name="toggle-business-hours" />
        </label>
        
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Facebook Business Manager ID
          <input v-model="fbID" type="text" name="toggle-business-hours" />
        </label>
        <label for="toggle-business-hours" class="toggle-input-wrap">
          WhatsApp Business Account ID
          <input v-model="accountID" type="text" name="toggle-business-hours" />
        </label>
        
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Namespace
          <input v-model="nameSpace" type="text" name="toggle-business-hours" />
        </label>
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Address
          <input v-model="addressW" type="text" name="toggle-business-hours" />
        </label>
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Description
          <input v-model="des" type="text" name="toggle-business-hours" />
        </label>
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Email
          <input v-model="emailW" type="text" name="toggle-business-hours" />
        </label>
        <label for="toggle-business-hours" class="toggle-input-wrap">
          Website
          <input v-model="websiteW" type="text" name="toggle-business-hours" />
        </label>

    </div>
    <div v-if="selectedTabKey === 'inbox_settings'" class="settings--content">
      <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.INBOX_UPDATE_TITLE')"
        :sub-title="$t('INBOX_MGMT.SETTINGS_POPUP.INBOX_UPDATE_SUB_TEXT')">
        <woot-avatar-uploader :label="$t('INBOX_MGMT.ADD.WEBSITE_CHANNEL.CHANNEL_AVATAR.LABEL')" :src="avatarUrl"
          class="settings-item" delete-avatar @change="handleImageUpload" @onAvatarDelete="handleAvatarDelete" />
        <woot-input v-model.trim="selectedInboxName" class="medium-9 columns settings-item" :label="inboxNameLabel"
        ...
        <woot-input v-if="isAPIInbox" v-model.trim="webhookUrl" class="medium-9 columns settings-item"
          :class="{ error: $v.webhookUrl.$error }" :label="
...
" @blur="$v.webhookUrl.$touch" />
        <woot-input v-if="isAWebWidgetInbox" v-model.trim="channelWebsiteUrl" class="medium-9 columns settings-item"
          :label="$t('INBOX_MGMT.ADD.WEBSITE_CHANNEL.CHANNEL_DOMAIN.LABEL')" :placeholder="
            $t('INBOX_MGMT.ADD.WEBSITE_CHANNEL.CHANNEL_DOMAIN.PLACEHOLDER')
          " />
        <woot-input v-if="isAWebWidgetInbox" v-model.trim="channelWelcomeTitle" class="medium-9 columns settings-item"
          :label="
...
" />
        ...
        <woot-input v-if="isAWebWidgetInbox" v-model.trim="channelWelcomeTagline" class="medium-9 columns settings-item"
          :label="
...
" />
        ...
        <div v-if="greetingEnabled" class="settings-item">
          <greetings-editor v-model.trim="greetingMessage" :label="
            $t(
...
" :richtext="!textAreaChannels" />
        ...
        <woot-submit-button v-if="isAPIInbox" type="submit" :disabled="$v.webhookUrl.$invalid"
          :button-text="$t('INBOX_MGMT.SETTINGS_POPUP.UPDATE')" :loading="uiFlags.isUpdatingInbox"
          @click="updateInbox" />
        <woot-submit-button v-else type="submit" :button-text="$t('INBOX_MGMT.SETTINGS_POPUP.UPDATE')"
          :loading="uiFlags.isUpdatingInbox" @click="updateInbox" />
      </settings-section>
    ...
    <div v-if="selectedTabKey === 'collaborators'" class="settings--content">
      <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.INBOX_AGENTS')"
    ...
    <div v-if="selectedTabKey === 'configuration'">
      <div v-if="isATwilioChannel" class="settings--content">
        <settings-section :title="$t('INBOX_MGMT.ADD.TWILIO.API_CALLBACK.TITLE')"
          :sub-title="$t('INBOX_MGMT.ADD.TWILIO.API_CALLBACK.SUBTITLE')">
          <woot-code :script="inbox.callback_webhook_url" lang="html"></woot-code>
        </settings-section>
      </div>
      <div v-if="isATataChannel" class="settings--content">
        <settings-section :title="$t('INBOX_MGMT.ADD.TATA.API_CALLBACK.TITLE')"
          :sub-title="$t('INBOX_MGMT.ADD.TATA.API_CALLBACK.SUBTITLE')">
          <woot-code :script="inbox.callback_webhook_url" lang="html"></woot-code>
        </settings-section>
      </div>
      <div v-else-if="isALineChannel" class="settings--content">
        <settings-section :title="$t('INBOX_MGMT.ADD.LINE_CHANNEL.API_CALLBACK.TITLE')"
          :sub-title="$t('INBOX_MGMT.ADD.LINE_CHANNEL.API_CALLBACK.SUBTITLE')">
          <woot-code :script="inbox.callback_webhook_url" lang="html"></woot-code>
        </settings-section>
      </div>
      <div v-else-if="isAViberChannel" class="settings--content">
        <settings-section :title="$t('INBOX_MGMT.ADD.VIBER.API_CALLBACK.TITLE')"
          :sub-title="$t('INBOX_MGMT.ADD.VIBER.API_CALLBACK.SUBTITLE')">
          <woot-code :script="inbox.callback_webhook_url" lang="html"></woot-code>
        </settings-section>
      </div>
      <div v-else-if="isAWebWidgetInbox">
        <div class="settings--content">
          <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.MESSENGER_HEADING')"
            :sub-title="$t('INBOX_MGMT.SETTINGS_POPUP.MESSENGER_SUB_HEAD')">
            <woot-code :script="inbox.web_widget_script"></woot-code>
          </settings-section>

          <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.HMAC_VERIFICATION')"
            :sub-title="$t('INBOX_MGMT.SETTINGS_POPUP.HMAC_DESCRIPTION')">
            <woot-code :script="inbox.hmac_token"></woot-code>
          </settings-section>
          <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.HMAC_MANDATORY_VERIFICATION')" :sub-title="
      ...
      <div v-else-if="isAPIInbox" class="settings--content">
        <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.INBOX_IDENTIFIER')"
      ...
      <div v-else-if="isAnEmailChannel">
        <div class="settings--content">
          <settings-section :title="$t('INBOX_MGMT.SETTINGS_POPUP.FORWARD_EMAIL_TITLE')"
...
export default {
  ...
  data() {
    return {
      ...
      selectedInboxName: '',
      selectedInboxNameTest: 'Tata Communications',
      messageLimit: '1K Customers/24hr',
      qualityRating: 'HIgh',
      defultLang: 'en',
      messageOnBehalf: 'Tata Communications',
      timeZone:'Asia/Kolkata',
      fbID:'1029825147410458',
      accountID:'109690298570217',
      nameSpace: '4e46b154_9778_414c_bcd3_4fa974a9a2ac',
      addressW:'Tata Communications Tower 4, Equinox Business Park, W, LBS Marg, Ambedkar Nagar, Kurla West, Kurla, Mumbai, India',
      des: 'Demonstration of Tata Communications WhatsApp integration.',
      websiteW: 'https://www.tatacommunications.com',
      emailW: '',      
      ...
      selectedTabIndex: 0,
      whatsppDisplayName: 'WhatsApp Display Name',
    };
  },
  computed: {
    ...mapGetters({
    ...
    tabs() {
      const visibleToAllChannelTabs = [
        ...
        {
          key: 'details',
          name: "Details",//this.$t('INBOX_MGMT.TABS.BUSINESS_HOURS'),
        },
