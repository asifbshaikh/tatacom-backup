...
const Whatsapp = ({
  ...
  const { messages } = intl;
  useEffect(() => {
    getConnectorActon(CommonEnums.TATA_SMS);
    addInboxCleanAction();
  }, []);
  ...
  const onSubmitForm = (values) => {
    const newParams = {
      ...
      channel_id: values.sms_connector,
      channel_type: CommonEnums.TATA_SMSC,
    };
  ...
  const handleChannelName = (setFieldValue, event) => {
    setFieldValue('sms_connector', event.target.value);
  };
            ...
            <Row>
              <Colxx xxs="12" md="6">
                <h2 className="mb-4 font-weight-bold">
                  <IntlMessages id="inboxes.sms.title" />
                </h2>
                <p>
                  <IntlMessages id="inboxes.sms.desc" />
                </p>
                <FormGroupCoustom
