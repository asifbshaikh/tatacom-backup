...
export const ReactTableWithPaginationCard = (propsTbl) => {
...
export const TringReactTable = ({ cols, items }) => {
  const newCols = cols.map((object) => {
    if (typeof object.Filter === 'string') {
      switch (object.Filter) {
        case 'NumberRangeColumnFilter':
          return { ...object, Filter: NumberRangeColumnFilter };
        case 'SelectColumnFilter':
          return { ...object, Filter: SelectColumnFilter };
  ...
  return (
    <div className="mb-4">
      ...
      <Table columns={newCols} data={items} divided />
    </div>
  );
};
